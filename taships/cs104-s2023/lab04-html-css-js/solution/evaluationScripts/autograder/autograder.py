        ...
        try:
            ...
            if bod is not None:
                ...
                try:
                    ...
                    if len(coldata) != 0 and len(data) % len(coldata) == 0:
...
scr = False
if soup.script is not None:
    if soup.script.get('src') is not None and 'script.js' in soup.script.get('src'):
        scr = True
    else:
...
try:
    ...
    if form is not None:
        ...
        try:
            ...
            if ['Email', 'Message', 'Name'] == sorted(map(lambda x: x.string if x.string is not None else '', labels)):
                mks6['form'] += 1
            else:
