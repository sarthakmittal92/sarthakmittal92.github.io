            ...
            if links is not None:
                ...
                valid = list(
                    filter(lambda x: x.text is not None and \
                        x.text.lower().replace(' ', '') in mapper and \
...
if len(scripts) > 1 and scripts[1] is not None:
    ...
    feedback['runFunc'] = 'Script not added'
if len(scripts) > 0 and scripts[0] is not None:
...
file = open('./public_html/script.js',
                  'r', encoding='utf-8').readlines()

try:
    _, res = js2py.run_file('./public_html/script.js')
    try:
        signal.alarm(10)
        res.isPrime(2)
        func = res.isPrime
        ...
        feedback['isPrime']['testcases'] = 'error encountered when running'
except Exception as e:
    feedback['isPrime']['testcases'] = 'error encountered when compiling'
